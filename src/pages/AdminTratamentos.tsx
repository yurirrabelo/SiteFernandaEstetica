...
import { Plus, Pencil, Eye, EyeOff, Trash2, Upload, Save, ArrowLeft } from "lucide-react";
...
const AdminTratamentos = () => {
  ...
  const openEdit = (t: Treatment) => {
    setEditingTreatment({ ...t }); setBenefitsText(t.benefits.join(", ")); setIsNew(false); setDialogOpen(true);
  };
  const openNew = () => {
    setEditingTreatment({ id: "", ...emptyTreatment, sort_order: treatments.length + 1 });
    setBenefitsText(""); setIsNew(true); setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    if (isNew) {
      ...
      const { error } = await supabase.from("treatments").update(payload).eq("id", editingTreatment.id);
      if (error) { toast.error("Erro ao salvar"); return; }
      toast.success("Tratamento atualizado!");
    }
    setDialogOpen(false); fetchTreatments();
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    toast.success("Excluído!"); fetchTreatments();
  };

  const toggleVisibility = async (t: Treatment) => {
    const { error } = await supabase.from("treatments").update({ visible: !t.visible }).eq("id", t.id);
    if (!error) { toast.success(t.visible ? "Oculto" : "Visível"); fetchTreatments(); }
  };
  ...
  const filtered = filterCategory === "all" ? treatments : treatments.filter(t => t.category === filterCategory);
    ...
    <Layout>
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl font-display font-semibold text-foreground">Tratamentos</h1>
                <p className="text-muted-foreground text-sm mt-1">Adicione, edite ou oculte tratamentos</p>
              </div>
            </div>
          ...
          <div className="flex gap-2 mb-6 flex-wrap">
            {[{ id: "all", label: "Todos" }, ...Object.entries(categoryLabels).map(([id, label]) => ({ id, label }))].map(cat => (
              <button key={cat.id} onClick={() => setFilterCategory(cat.id)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${filterCategory === cat.id ? "gradient-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}>{cat.label}</button>
            ))}
          </div>

          <div className="grid gap-4">
            {filtered.map(treatment => (
              <div key={treatment.id} className={`bg-card rounded-xl p-4 sm:p-6 shadow-soft flex flex-col sm:flex-row items-start sm:items-center gap-4 transition-opacity ${!treatment.visible ? "opacity-50" : ""}`}>
                <div className="w-full sm:w-20 h-32 sm:h-20 rounded-lg overflow-hidden bg-muted flex-shrink-0">
                  {treatment.image_url ? <img src={treatment.image_url} alt={treatment.title} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-muted-foreground text-xs">Sem imagem</div>}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-display font-semibold text-foreground truncate">{treatment.title}</h3>
                    {!treatment.visible && <span className="text-xs bg-muted px-2 py-0.5 rounded-full text-muted-foreground">Oculto</span>}
                  </div>
                  <p className="text-sm text-muted-foreground">{categoryLabels[treatment.category]} · {treatment.duration}</p>
                </div>
                <div className="flex gap-2 w-full sm:w-auto">
                  <Button size="sm" variant="outline" onClick={() => openEdit(treatment)} className="flex-1 sm:flex-none"><Pencil size={14} className="mr-1" />Editar</Button>
                  <Button size="sm" variant="outline" onClick={() => toggleVisibility(treatment)} className="flex-1 sm:flex-none">
                    {treatment.visible ? <EyeOff size={14} className="mr-1" /> : <Eye size={14} className="mr-1" />}
                    {treatment.visible ? "Ocultar" : "Mostrar"}
                  </Button>
                  <Button size="sm" variant="destructive" onClick={() => handleDelete(treatment.id)} className="flex-1 sm:flex-none"><Trash2 size={14} /></Button>
                </div>
              </div>
            ))}
          </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle className="font-display">{isNew ? "Novo Tratamento" : "Editar Tratamento"}</DialogTitle></DialogHeader>
          {editingTreatment && (
            <div className="space-y-4 mt-4">
              <div><Label>Título</Label><Input value={editingTreatment.title} onChange={e => setEditingTreatment({ ...editingTreatment, title: e.target.value })} className="mt-1" /></div>
              <div><Label>Descrição</Label><Textarea value={editingTreatment.description} onChange={e => setEditingTreatment({ ...editingTreatment, description: e.target.value })} className="mt-1" rows={3} /></div>
              ...
              <div>
                ...
                <div className="mt-1 space-y-2">
                  {editingTreatment.image_url && <img src={editingTreatment.image_url} alt="Preview" className="w-full h-40 object-cover rounded-lg" />}
              ...
              <div className="flex items-center justify-between"><Label>Visível no site</Label><Switch checked={editingTreatment.visible} onCheckedChange={v => setEditingTreatment({ ...editingTreatment, visible: v })} /></div>
