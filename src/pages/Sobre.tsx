      ...
      <section className="pt-32 pb-20 gradient-hero">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl mx-auto text-center">
            <h1 className="text-4xl md:text-5xl font-display font-semibold text-foreground mb-6">Sobre <span className="text-gradient-primary">Fernanda Concolatto</span></h1>
            <p className="text-lg text-muted-foreground leading-relaxed">Há 12 anos transformando beleza, corpo e alma. Estética avançada com abordagem holística e cuidado integral.</p>
          </motion.div>
        </div>
      </section>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="space-y-6">
              ...
              <div className="space-y-4 text-muted-foreground leading-relaxed">
                <p>Paranaense de alma curiosa e coração aquariano. Há 18 anos escolhi a estética como caminho, e desde então sigo me aperfeiçoando com amor e propósito.</p>
                <p>Comecei minha jornada em Curitiba, em clínicas de referência, e quando voltei para SC, senti que era hora de criar algo meu. Assim nasceu minha própria clínica, unindo conhecimento técnico com o poder das terapias e do autodesenvolvimento 🌿💆‍♀️</p>
                <p>Amo o que faço e acredito no cuidado integral — físico, emocional, mental e espiritual. Sou apaixonada por animais (tenho uma cachorrinha chamada Loba 🐶), por plantas, cachoeiras e tudo que me conecta com a natureza e a paz interior 🌺💧</p>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {values.map((value, index) => (
              <motion.div key={value.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="text-center p-6">
                <div className="w-16 h-16 mx-auto rounded-2xl gradient-primary flex items-center justify-center mb-4"><value.icon size={28} className="text-primary-foreground" /></div>
        ...
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="order-1 lg:order-2 space-y-6">
              ...
              <h2 className="text-3xl md:text-4xl font-display font-semibold text-foreground">Fernanda Tagliari Concolatto</h2>
              <div className="space-y-4 text-muted-foreground leading-relaxed">
                <p>Esteticista há 18 anos, com especializações em estética avançada e terapias holísticas. Sua paixão pelo cuidado integral a levou a criar um espaço único em Itapema.</p>
                <p>"Acredito no cuidado integral — físico, emocional, mental e espiritual. Cada tratamento é uma oportunidade de reconexão consigo mesma." ✨</p>
              </div>
              <div className="flex flex-wrap gap-4 pt-4">
                <div className="bg-primary/10 px-4 py-2 rounded-full text-primary text-sm font-medium">Pós Operatório</div>
                <div className="bg-secondary/10 px-4 py-2 rounded-full text-secondary text-sm font-medium">Botox</div>
                <div className="bg-accent/10 px-4 py-2 rounded-full text-accent text-sm font-medium">Beleza Facial e Corporal</div>
